...
function mustFixHiatus(contentAfterDet) {
    if (!contentAfterDet) {
        return false;
    }
    const trimmedContent = contentAfterDet.split(' ')[0].trim();
    if ((0, french_contractions_1.isHAspire)(trimmedContent)) {
        return false;
    }
    const mustHiatus = hiatusStarts.some((item) => trimmedContent.startsWith(item));
    return mustHiatus;
}
function getDet({ detType, genderOwned, numberOwned, numberOwner, adjectiveAfterDet, contentAfterDet, forceDes, personOwner, }) {

...
Compiler.prototype = {
    ...
    compile: function () {
        ...
        return (buildRuntime(this.runtimeFunctionsUsed) +
            'function ' +
    ...
    visitMixinBlock: function (block) {
        if (this.pp)
            this.buf.push("pug_indent.push('" + Array(this.indents + 1).join(this.pp) + "');");
    ...
    visitMixin: function (mixin) {
        ...
        if (mixin.call) {
            ...
            if (pp)
                this.buf.push("pug_indent.push('" + Array(this.indents + 1).join(pp) + "');");
    ...
    visitTag: function (tag, interpolated) {
        this.indents++;
        const name = tag.name, pp = this.pp, 
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        self = this;
        function bufferName() {
            if (interpolated)
                self.bufferExpression(tag.expr, false);
            else
                self.buffer(name);
        }
    ...
    visitAttributes: function (attrs, attributeBlocks) {
        if (attributeBlocks.length) {
            ...
            if (attributeBlocks.length > 1) {
                this.bufferExpression(this.runtime('attrs') +
                    '(' +
                    this.runtime('merge') +
                    '([' +

...
function getAdjectiveInfo(adjective, contractsData) {
    ...
    for (const gender of ['M', 'F']) {
        for (const number of ['S', 'P']) {
            const agreedAdj = (0, french_adjectives_1.agree)(adjective, gender, number, null, false, contractsData);
            res[gender + number] = agreedAdj;
            if ((0, french_adjectives_1.getChangeant)(agreedAdj)) {
                res[agreedAdj] = (0, french_adjectives_1.getChangeant)(agreedAdj);
            }
        }
    }
    return res;
    ...
    if (adjectivesInfo) {
        ...
        if (adjectivesInfo[adjective] && adjectivesInfo[adjective][key]) {
            ...
            if (isBeforeNoun && number === 'S' && adjectivesInfo[adjective][agreedAdj] != null) {
                if ((0, french_contractions_1.contracts)(noun, contractsData)) {
...
function agreeAdjective(adjectivesInfo, adjective, gender, number, noun, isBeforeNoun, contractsData) {
    ...
    if (fromAdjectivesInfo) {
        ...
        return (0, french_adjectives_1.agree)(adjective, gender, number, noun, isBeforeNoun, contractsData);
    }
}

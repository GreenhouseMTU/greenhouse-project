...
function getCaseNumber(wordsList, wordInfoKey, numberKey, word) {
    if (wordsList &&
        wordsList[word] &&
        wordsList[word][wordInfoKey] &&
        wordsList[word][wordInfoKey][numberKey]) {
        return wordsList[word][wordInfoKey][numberKey];
    }
    else {
        return null;
    }
...
function getCaseGermanWord(wordsListExceptions, wordsList, word, germanCase, number) {
    ...
    const wordInfoKey = casesMapping[germanCase];
    const numberKey = number == 'S' ? 'SIN' : 'PLU';
    const caseNumber = getCaseNumber(wordsListExceptions, wordInfoKey, numberKey, word) ||

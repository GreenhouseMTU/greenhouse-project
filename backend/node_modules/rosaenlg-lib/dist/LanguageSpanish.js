...
class LanguageSpanish extends LanguageImpl_1.LanguageImpl {
    ...
    getDet(det, params) {
        return (0, spanish_determiners_1.getDet)(det, params.genderOwned, params.numberOwned, params.after, params.dist);
    }
    getAgreeAdj(adjective, gender, number, _subject, params) {
        return (0, spanish_adjectives_wrapper_1.agreeAdjective)(this.getDictManager().getAdjsData(), adjective, gender, number, params && params.adjPos === 'BEFORE' ? true : false);
    }
    getWordGender(word) {
        return (0, spanish_words_1.getGenderSpanishWord)(this.getDictManager().getWordData(), word);
    }
    getOrdinal(val, gender) {
        return (0, ordinal_spanish_1.default)(val, gender == 'M' ? 'male' : 'female');
    }
    getSubstantive(subst, number) {
        if (number === 'S') {
            ...
            return (0, spanish_words_1.getPluralSpanishWord)(this.getDictManager().getWordData(), subst);
        }
    }
    getConjugation(_subject, verb, tense, person, _conjParams, embeddedVerbs) {
        // one of verbsSpecificList and conjFctEs is always null: it's one or the other
        return (0, spanish_verbs_wrapper_1.getConjugation)(embeddedVerbs, verb, this.solveTense(tense), this.mapPersonToNumber0to5(person));
    }
    isPlural(val) {

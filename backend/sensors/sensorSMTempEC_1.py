...
def get_sensorSMTempEC1Week():
    ...
    sensorSMTempEC1WeekValues = db.session.query(Sensor_SMTempEC_1).filter(
        Sensor_SMTempEC_1.datetime >= start_date_week.strftime('%Y-%m-%d %H:%M:%S'),
        Sensor_SMTempEC_1.datetime <= end_date_week.strftime('%Y-%m-%d %H:%M:%S')
    ).all()

    hourly_avg_values = {}
    for entry in sensorSMTempEC1WeekValues:

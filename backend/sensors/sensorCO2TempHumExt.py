...
def get_sensorCO2TempHumExtWeek():
    ...
    data_entries = db.session.query(Sensor_CO2TempHum_ext).filter(
        Sensor_CO2TempHum_ext.datetime >= start_date,
        Sensor_CO2TempHum_ext.datetime <= end_date
    ).all()
...
def get_sensorCO2TempHumExtMonth():
    ...
    sensorCO2TempHumExtMonthValues = db.session.query(Sensor_CO2TempHum_ext).filter(
        Sensor_CO2TempHum_ext.datetime >= start_date_month,
        Sensor_CO2TempHum_ext.datetime <= current_date
    ).all()
    ...
    for entry_date in daily_avg_values:
        for start_hour, end_hour, end_minute, end_second in part_intervals:
            ...
            part_entries = [
                entry for entry in sensorCO2TempHumExtMonthValues

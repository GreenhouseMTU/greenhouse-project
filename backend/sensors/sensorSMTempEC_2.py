...
def get_sensorSMTempEC2DayAverage():
    ...
    hourly_avg_values = {}
    for entry in sensorSMTempEC2DayAverageValues:
...
def get_sensorSMTempEC2Month():
    ...
    daily_avg_values = {}
    for entry in sensorSMTempEC2MonthValues:
    ...
    for date, data in daily_avg_values.items():
        ...
        for start_hour, end_hour, end_minute, end_second in part_intervals:
            part_values = [val for val in sensorSMTempEC2MonthValues if start_hour <= val.datetime.hour <= end_hour and val.datetime.date() == date]

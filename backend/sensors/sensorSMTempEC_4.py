...
from models import Sensor_SMTempEC_4
from db import db

sensor_smtempec_4_app = Blueprint('sensor_smtempec_4_app', __name__)

current_date = datetime.date.today()

# This endpoint is used to get the latest value
@sensor_smtempec_4_app.route('/api/sensors/sensor_smtempec_4/latest', methods=['GET'])
@jwt_required()
def get_sensorSMTempEC4():
    # utilise directement db.session.query(...)
    sensorSMTempEC4LatestValue = db.session.query(Sensor_SMTempEC_4).order_by(Sensor_SMTempEC_4.id.desc()).first()
    if sensorSMTempEC4LatestValue is not None:
        serialized_data = sensorSMTempEC4LatestValue.serialize()
...
@sensor_smtempec_4_app.route('/api/sensors/sensor_smtempec_4/day/average', methods=['GET'])
@jwt_required()
def get_sensorSMTempEC4DayAverage():
    # utilise directement db.session.query(...)
    current_date = datetime.datetime.now()
    formatted_date = current_date.strftime('%Y-%m-%d')
    sensorSMTempEC4DayAverageValues = db.session.query(Sensor_SMTempEC_4).filter(Sensor_SMTempEC_4.datetime.like(f'%{formatted_date}%')).all()

    hourly_avg_values = {}
    for entry in sensorSMTempEC4DayAverageValues:
...
# Nouveau endpoint pic-average pour Sensor_SMTempEC_4

@sensor_smtempec_4_app.route('/api/sensors/sensor_smtempec_4/day/pic-average', methods=['GET'])
@jwt_required()
def get_sensorSMTempEC4PicAverage():
    # utilise directement db.session.query(...)
    now = datetime.datetime.now()
    formatted_date = now.strftime('%Y-%m-%d')

    data_entries = db.session.query(Sensor_SMTempEC_4).filter(Sensor_SMTempEC_4.datetime.like(f'%{formatted_date}%')).all()
...
@sensor_smtempec_4_app.route('/api/sensors/sensor_smtempec_4/day', methods=['GET'])
@jwt_required()
def get_sensorSMTempEC4ExtDay():
    # utilise directement db.session.query(...)
    formatted_date = current_date.strftime('%Y-%m-%d')
    sensors = db.session.query(Sensor_SMTempEC_4).filter(Sensor_SMTempEC_4.datetime.like(f'%{formatted_date}%')).all()
...
@sensor_smtempec_4_app.route('/api/sensors/sensor_smtempec_4/week', methods=['GET'])
@jwt_required()
def get_sensorSMTempEC4Week():
    offset = int(request.args.get('offset', 0))
    now = datetime.datetime.now() + datetime.timedelta(days=offset * 7)
    start_date_week = now - datetime.timedelta(days=7)
    end_date_week = now

    sensorSMTempEC4WeekValues = db.session.query(Sensor_SMTempEC_4).filter(
        Sensor_SMTempEC_4.datetime >= start_date_week.strftime('%Y-%m-%d %H:%M:%S'),
        Sensor_SMTempEC_4.datetime <= end_date_week.strftime('%Y-%m-%d %H:%M:%S')
    ).all()

    hourly_avg_values = {}
    for entry in sensorSMTempEC4WeekValues:
...
@sensor_smtempec_4_app.route('/api/sensors/sensor_smtempec_4/month', methods=['GET'])
@jwt_required()
def get_sensorSMTempEC4Month():
    # utilise directement db.session.query(...)
    current_date = datetime.datetime.now()
    start_date_month = current_date - datetime.timedelta(days=30)
    
    sensorSMTempEC4MonthValues = db.session.query(Sensor_SMTempEC_4).filter(
        Sensor_SMTempEC_4.datetime >= start_date_month.strftime('%Y-%m-%d %H:%M:%S'),
        Sensor_SMTempEC_4.datetime <= current_date.strftime('%Y-%m-%d %H:%M:%S')
    ).all()

    daily_avg_values = {}
    for entry in sensorSMTempEC4MonthValues:
    ...
    for date, data in daily_avg_values.items():
        ...
        for start_hour, end_hour, end_minute, end_second in part_intervals:
            part_values = [val for val in sensorSMTempEC4MonthValues if start_hour <= val.datetime.hour <= end_hour and val.datetime.date() == date]
